...
class默认的访问级别是private，
struct默认的访问级别是public。
...
**弊**：
* 一个不少人承认的说法是友元牺牲了封装性与可维护性。（事实上并没有，[详见《How Non-Member Functions Improve Encapsulation
...
## 练习7.27 : [头文件](07_27.h) | [主函数](07_27.cpp)
...
getSize(34);
```
很明显这样调用会让人困惑，函数实际上会初始化一个拥有34个元素的vector的临时量，然后返回34。但是这样没有任何意义。而 string 则不同，string的单参数构造函数的参数是 ``const char * ``，因此凡是在需要用到 string 的地方都可以用 ``const char *`` 来代替（字面值就是 const char *）。如：
